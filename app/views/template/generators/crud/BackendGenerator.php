...
namespace app\views\template\generators\crud;
 ...
 * @since 2.0
 */
class BackendGenerator extends \yii\gii\Generator
{
    public $modelClass;
    ...
    public function generateActiveField($attribute)
    {
        ...
        if ($tableSchema === false || ! isset($tableSchema->columns[$attribute])) {
            if (preg_match('/^(password|pass|passwd|passcode)$/i', $attribute)) {
                return "\$form->field(\$model, '$attribute')->passwordInput())->label(Yii::t('app','$attribute'));";
            } else {
                return "\$form->field(\$model, '$attribute')->label(Yii::t('app','$attribute'));";
            }
        }
        $column = $tableSchema->columns[$attribute];
        if ($column->phpType === 'boolean') {
            return "\$form->field(\$model, '$attribute')->checkbox()->label(Yii::t('app','$attribute'));";
        } elseif ($column->type === 'text') {
            return "\$form->field(\$model, '$attribute')->textarea(['rows' => 6])->label(Yii::t('app','$attribute'));";
        } else {
            ...
            if (is_array($column->enumValues) && count($column->enumValues) > 0) {
                ...
                return "\$form->field(\$model, '$attribute',['options' => ['class' => '']])->dropDownList(" . preg_replace("/\n\s*/", ' ', VarDumper::export($dropDownOptions)) . ", ['prompt' => Yii::t('app','select')])->label(Yii::t('app','$attribute'));";
            } elseif ($column->phpType !== 'string' || $column->size === null) {
                return "\$form->field(\$model, '$attribute')->$input(['maxlength' => true])->label(Yii::t('app','$attribute'));";
            } else {
                return "\$form->field(\$model, '$attribute')->$input(['maxlength' => true])->label(Yii::t('app','$attribute'));";
            }
        }
