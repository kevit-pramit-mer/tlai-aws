...
(function (factory) {
    ...
    'use strict';
    $.widget("ecosmob.assignSupervisors", {
        _create: function () {
            var self = this;
            $.proxy(self._leftAssign, self)();
            $.proxy(self._rightAssign, self)();
        ...
        _leftAssign: function () {
            var self = this;
            $(document).on('click', '#btnLeftSuperviosr', function (e) {
                var agents = [];
                var agentsList = $('#selectedSupervisors').val().split(',');
                var ind;
                $.each($('#assignedSupervisors option:selected'), function () {
                    agents.push($(this).val());
                    ind = agentsList.indexOf($(this).val());
                    if (ind !== -1) {
                        agentsList.splice(ind, 1); // The second parameter is the number of elements to remove.
                    }
                    $(this).remove().appendTo('#availableSupervisors');
                });
                agentsList = agentsList.join(',');
                $('#selectedSupervisors option:selected').removeAttr("selected");
                $('#selectedSupervisors').val(agentsList);
                e.preventDefault();
            });
        },
        ...
        _rightAssign: function () {
            var self = this;
            $(document).on('click', ('#btnRightSupervisor', self.elements), function (e) {
                $(document).on('click', '#btnRightSupervisor', function (e) {
                    var agents = [];
                    $.each($("#availableSupervisors option:selected"), function () {
                        agents.push($(this).val());
                        $(this).remove().appendTo('#assignedSupervisors');
                    });
                    var agentsList = $("#selectedSupervisors").val();
                    if (agentsList) {
                        agentsList = agentsList.split(',');
                        agents = $.merge(agentsList, agents);
                    }
                    agentsList = agents.join(',');
                    $("#assignedSupervisors option:selected").removeAttr("selected");
                    $("#selectedSupervisors").val(agentsList);
                    e.preventDefault();
                });
            });
...
$(document).ready(function () {
    $("#campaign-form-1").assignAgents();
    $("#campaign-form-1").assignSupervisors();
});
