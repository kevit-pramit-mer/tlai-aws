...
class LoginForm extends Model
{
    ...
    public function validatePassword($attribute)
    {
        if (!$this->hasErrors()) {
            ...
            if (!$user){
                $this->addError($attribute, AuthModule::t('auth', 'user_not_exit'));
            }else {
    ...
    public function checkActivationStatus($attribute)
    {
        if (!$this->hasErrors()) {
            if (!$this->loginAsExtension) {
                ...
                if ($user) {
                    if (!$user->adm_status) {
                        $this->addError($attribute, AuthModule::t('auth', 'user_is_inactive'));
                    }
                }
            }else{
                $extension = Extension::findOne(['em_extension_number' => $this->username]);
                if ($extension) {
                    if (!$extension->em_status) {
                        $this->addError($attribute, AuthModule::t('auth', 'user_is_inactive'));
                    }
                }
            }
