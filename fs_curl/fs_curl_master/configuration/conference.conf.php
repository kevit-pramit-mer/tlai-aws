...
//	$this -> write_advertises();
    ...
    */
	...
	$prof = $this -> request['profile_name'];
        $query = "SELECT * FROM conference_profiles where profile_name collate utf8mb4_unicode_ci = '$prof'";
	//$query = "CALL get_conference_profile('$prof')";	
    ...
    */
    private function get_controls_array() {
		...
		$query = sprintf(
		    "SELECT * FROM ct_conference_controls where cc_conf_group = '$cont'"
	);	
        $res = $this -> db -> query($query);
        if (FS_PDO::isError($res)) {
            $this -> comment($query);
    ...
    private function write_controls() {
        ...
        foreach ($controls_array as $group_name => $control_data) {
            ...
            foreach ($control_data as $controls) {
                ...
                $this -> xmlw -> writeAttribute('action', $controls['cc_action']);
                $this -> xmlw -> writeAttribute('digits', $controls['cc_digits']);
